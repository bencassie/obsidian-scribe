...
If you switch between WSL and Windows, you need **two config files** in your vault's `.claude/` folder:

| File | Used By | Path Format |
|------|---------|-------------|
| `settings.json` | Windows | `C:/Users/...` |
| `settings.local.json` | WSL | `/mnt/c/Users/...` |

Both files should have the same structure, just with platform-appropriate paths.

**Example `settings.json` (Windows):**
```json
{
  "extraKnownMarketplaces": {
    "obsidian-scribe": {
...
**Example `settings.local.json` (WSL):**
```json
{
  "extraKnownMarketplaces": {
    "obsidian-scribe": {
